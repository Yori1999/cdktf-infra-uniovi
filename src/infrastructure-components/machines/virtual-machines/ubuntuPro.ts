...
export class UbuntuPro extends UbuntuBase {
    ...
    private checkUbuntuProTokenProvided(): void {
        if (!this.ubuntuProToken) {
            throw new Error("Ubuntu Pro token has not been set. You need to provide an Ubuntu Pro token, by setting UBUNTU_PRO_TOKEN with the --var option when calling cdktf deploy");
        }
    }
